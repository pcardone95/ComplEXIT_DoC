function parse_data_4ECI_bas_fullrec
%% Parse data for Minji
% This version directly goes to check the baseline + full recording
%% Set up things
% dose_l = {'0.15', '0.30', '0.45', '0.60', '0.75'};

% 60 Channels
load('C:\Users\Paolo\OneDrive - Universite de Liege\Documents\GitHub\ComplEXIT_DoC\Ketamine\eximia_addchn.mat',...
    'eximia_addchn') % Structure with good additional channels
channel_name = {'Fp1', 'Fpz', 'Fp2', 'AF1', 'AFz', 'AF2', ...
'F7' ,'F3', 'F1', 'Fz','F2', 'F4', 'F8', 'FT9', ...
'FT7', 'FC5', 'FC3', 'FC1', 'FCz', 'FC2', 'FC4', 'FC6', 'FT8', 'FT10', ...
'T7','C5', 'C3', 'C1',...
'Cz',...% <-- check this
'C2','C4','C6','T8', 'TP9', ...% 
'TP7', 'CP5',  'CP3', 'CP1', 'CPz', 'CP2', 'CP4', 'CP6', 'TP8', 'TP10', ...%
'P9', 'P7', 'P3', 'P1', 'Pz', 'P2', 'P4', 'P8', 'P10', 'PO3', 'POz',...
'PO4','O1','Oz','O2', 'Iz'};
data_wd = 'C:\Users\Paolo\OneDrive - Universite de Liege\Bureau\OldComputer\D\Complexit_doc\Ketamine';

for pati = 1:3
    for sessi = 1:2
        filepath = fullfile(data_wd, sprintf('PilotKET0%i\\Sess%i\\Baseline\\',pati, sessi));
        
        % for dosei = 1:length(dose_l)
        cd(filepath)
        file_l = ls('*.set');
        
        % Iterate each line to find what we want
        for namei = 1:size(file_l, 1)
            file_c = strtrim(file_l(namei,:));
            file_c = strsplit(file_c(1:end-4),'_');
            if not(any([strcmp('clean', file_c) strcmp('orig', file_c) strcmp('epochs', file_c)]))
                break
            end
        end
        name_sess = strjoin(file_c,'_');
        
        EEG = pop_loadset('filename',sprintf('%s.set', name_sess),'filepath',filepath);
        
        % Control where are the good electrodes
        [~, channels_ECI]= ismember(channel_name, {EEG.chanlocs.labels});
        channels_ECI = nonzeros(channels_ECI);
        
        % 60 Eximia channels
        EEG.chanlocs = [EEG.chanlocs, eximia_addchn];
        
        % Create 0 data
        EEG.data = [EEG.data(:,:,:); zeros(length(eximia_addchn),2500,size(EEG.data(:,:,:),3))];
        EEG.nbchan = EEG.nbchan + length(eximia_addchn);
        %
        % Interpolate
        EEG = eeg_interp(EEG, 128:EEG.nbchan);
        
        EEG = pop_select(EEG, 'channel', [channels_ECI' 128:132]);
        cd('C:\Users\Paolo\OneDrive - Universite de Liege\Bureau\Minji\Ketamine_Pilot\Baseline_full')
        pop_saveset(EEG,...
            'filename',sprintf('P%i_S%i_bas_60ch.set', pati, sessi));
    end
end

% Full recording
for pati = 1:3
    for sessi = 1:2
        filepath = fullfile(data_wd, sprintf('PilotKET0%i\\Sess%i\\',pati, sessi));
        
        % for dosei = 1:length(dose_l)
        cd(filepath)
        
        EEG = pop_loadset('filename',sprintf('PK%i_S%i_exp.set', pati, sessi),'filepath',filepath);
        
        % Control where are the good electrodes
        [~, channels_ECI]= ismember(channel_name, {EEG.chanlocs.labels});
        channels_ECI = nonzeros(channels_ECI);
        
        % 60 Eximia channels
        EEG.chanlocs = [EEG.chanlocs, eximia_addchn];
        
        % Create 0 data
        EEG.data = [EEG.data(:,:,:); zeros(length(eximia_addchn),2500,size(EEG.data(:,:,:),3))];
        EEG.nbchan = EEG.nbchan + length(eximia_addchn);
        %
        % Interpolate
        EEG = eeg_interp(EEG, 128:EEG.nbchan);
        
        EEG = pop_select(EEG, 'channel', [channels_ECI' 128:132]);
        cd('C:\Users\Paolo\OneDrive - Universite de Liege\Bureau\Minji\Ketamine_Pilot\Baseline_full')
        pop_saveset(EEG,...
            'filename',sprintf('P%i_S%i_full_60ch.set', pati, sessi));
    end
end


end
