...
LZC_graph_summary <- data_PerCon_summarize %>% 
  ...
  geom_errorbar(aes(ymin=mean_LZC-sd_LZC/sqrt(127), ymax=mean_LZC+sd_LZC/sqrt(127)), width=.2) + 
  ylab('') +xlab('Concentration (µg/ml)')+ theme_bw() + 
  theme(text = element_text(size =10))+ 
...
ggarrange(
  ggarrange(LZC_graph_grid, data_ECIAr_grid, data_ECIAw_grid, nrow = 1, 
            labels = c('A', 'B', 'C'), font.label = list(size = 20), 
            common.legend = TRUE, legend = "right"), # First row
  ggarrange(LZC_graph_summary_grid, ECI_aro_graph_grid, ECI_awa_graph_grid, nrow = 1), # Second row
  nrow = 2)
ggsave(paste0(today(),"_EEGresults2.tiff"), device="tiff", path=paper_path, dpi=300)
ggsave(paste0(today(),"_EEGresults2.jpg"), device="jpg", path=paper_path, dpi=300)
...
ggsave(paste0(today(),"_EEGresults.tiff"), device="tiff", width=15, height=8.25, units="in", path=paper_path, dpi=300)
...
seconds_graph_v2 <- data_seconds %>% ggplot(aes(x=Time, y=Index, shape=Diagnosis, color=Condition,
  ...
  ggtitle('SECONDs score') +ylab('') +xlab('Time (min)')+ theme_bw() +
  theme(text = element_text(size = 25), plot.title = element_text(hjust = 0.5), axis.text.x = element_text(size = 18))   +  
  scale_y_continuous(breaks = seq(0,8,1), limits = c(-0.1, 8.1)) + geom_vline(xintercept = 4.5, linetype="dashed", 
                                                                              color = "black", size=2)+
...
MAS_graph <- data_MAS %>% 
  ...
  geom_point(size=5)+ geom_line() + 
  ylab('') +xlab('Time (min)')+ ggtitle('Modified Ashworth Scale score') + theme_bw() +
...
MAS_graph_mod_grid <- MAS_graph_mod + theme(legend.position='none') + facet_nested(Condition ~ Subject + Side)
#ggsave(paste0(today(),"_KET_MASS_mod.tiff"), device="tiff", width=30, height=20, units="cm", path=paper_path, dpi=300)

# Figure 2 - Behav results####
ggarrange(seconds_graph_grid, 
          MAS_graph_mod_grid,  
          labels = c('A', 'B'), font.label = list(size = 30), nrow = 2)

ggsave(paste0(today(),"_Behavresults.tiff"), device="tiff", width=14, height=14, units="in", path=paper_path, dpi=300)
